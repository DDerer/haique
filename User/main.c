...
float Motor_Speed_Get(uint8_t motor_id)
{
	...
	{
		case 0:
			encoder_diff = TIM_GetCounter(TIM1);
			TIM_SetCounter(TIM1, 0);
			break;
		case 1:
			encoder_diff = TIM_GetCounter(TIM4);
			TIM_SetCounter(TIM4, 0);
			break;
		case 2:
			encoder_diff = TIM_GetCounter(TIM8);
			TIM_SetCounter(TIM8, 0);
			break;		
	}
	float dist = (encoder_diff / (ENCODER_LINES *16.0f * GEAR_RATIO)) * wheel_circumference;
...
void Motor_Control(uint8_t idx)
{
	...
	out = motor_pid[idx].Output; // 更新PID输出值
	//设置方向
...
void Emergency_Stop() {
    ...
    for(int i=0; i<3; i++) {
        ...
        motor_pid[i].Setpoint = 0;
    }
}
//三角形运动
void Move_Triangle() {
    for (int i = 0; i < 10; i++) {
        Motor_Speed_Update(-5.0f * sqrt(3.0f), 5.0f, 0);
        Delay_ms(100);
    }
	Emergency_Stop();	
    for (int i = 0; i < 10; i++) {
...
void Move_Square() {
    for (int i = 0; i < 10; i++) {
        Motor_Speed_Update(-15, 0, 0);
        Delay_ms(100);
    }
	Emergency_Stop();
    for (int i = 0; i < 10; i++) {
        Motor_Speed_Update(0, 15, 0);
        Delay_ms(100);
    }
	Emergency_Stop();
    for (int i = 0; i < 10; i++) {
...
void Move_Round() {
    // 运动参数配置
    float r = 0.18f;                  // 半径0.18米（适中大小）
    float l_speed = 0.15f;            // 线速度0.15m/s（较慢速度）
    uint32_t steps = 24;              // 增加步数保证平滑
    float circumference = 2 * 3.1415926f * r;
    
    // 时间控制（总时间4.5秒）
    uint32_t total_time = 4500;       // 4500ms = 4.5秒
    uint32_t step_time = total_time / steps;  // 每步约187ms
    
    // PWM计算（限制最大50%输出）
    float speed_pwm = (l_speed / MAX_LINEAR_SPEED) * 100.0f;
    if(speed_pwm > 50.0f) speed_pwm = 50.0f;
    for(uint32_t i = 0; i < steps; i++) {
        float angle = 2 * 3.1415926f * i / steps;
        
        // 速度分量计算
        float vx_pwm = speed_pwm * sinf(angle);
        float vy_pwm = speed_pwm * cosf(angle);
        
        Motor_Speed_Update(vx_pwm, vy_pwm, 0);
        Delay_ms(step_time);
...
int main()
{
    ...
    TIM3_Config();
	Encoder_Init();
	PID_Init(&motor_pid[0], 0.15f, 0.8f, 0.7f,0);  // 电机1
	PID_Init(&motor_pid[1], 0.15f, 0.8f, 0.7f,0);  // 电机2  
	PID_Init(&motor_pid[2], 0.15f, 0.8f, 0.7f,0);  // 电机3	
	while(1)
	{	
		Move_Round();
	}
...
{	
	if(TIM_GetITStatus(TIM3,TIM_IT_Update)==SET)
	{
		...
		Motor_Control(1);
		Motor_Control(2);
//		Serial_Printf("%d",TIM_GetCounter(TIM1));
